    ...
    <main className="flex-1 overflow-auto bg-[#fafafa]">
      <div className="max-w-7xl mx-auto px-8 py-8">
        ...
        <header className="mb-10">
          ...
          <h1 className="font-serif text-4xl font-medium text-neutral-900 tracking-tight">
            {getGreeting()}
          </h1>
        </header>
        ...
        <div className="grid grid-cols-12 gap-5">
          {/* Row 1 */}
          {/* Deadlines */}
          <div className="col-span-4 bg-white border border-neutral-200 rounded-xl overflow-hidden flex flex-col">
            <DeadlinesWidget
              deadlines={deadlines}
          ...
          {/* Schedule */}
          <div className="col-span-5 bg-white border border-neutral-200 rounded-xl overflow-hidden flex flex-col">
            <ScheduleWidget
          ...
          {/* Tasks */}
          <div className="col-span-3 bg-white border border-neutral-200 rounded-xl overflow-hidden flex flex-col">
            <TasksWidget boards={boards} onNavigate={handleBoardNavigate} />
          </div>

          {/* Row 2 */}
          {/* Quick Capture */}
          <div className="col-span-6 bg-white border border-neutral-200 rounded-xl overflow-hidden flex flex-col">
...
function WidgetHeader({ title, onAdd }) {
  return (
    <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between flex-shrink-0">
      ...
      {onAdd && (
        <button onClick={onAdd} className="text-neutral-300 hover:text-neutral-500 transition-colors">
          <Plus size={15} />
        </button>
      )}
    </div>
          ...
          <div className="p-6 border-b border-neutral-100 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setIsAdding(false); setNewDeadline({ title: '', date: '' }); }}
                className="flex-1 py-2 text-sm text-neutral-400 hover:text-neutral-600 transition-colors">Cancel</button>
            </div>
          </div>
        )}
        ...
        {sorted.length === 0 && !isAdding ? (
          ...
          <div className="divide-y divide-neutral-100">
            {sorted.map((d) => {
              ...
              return (
                <div key={d.id} className={`px-6 py-5 flex items-center justify-between group ${isOverdue ? 'opacity-40' : ''}`}>
                  <div>
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className={`font-serif text-4xl font-medium tabular-nums ${isUrgent ? 'text-amber-600' : 'text-neutral-300'}`}>
                      {isOverdue ? Math.abs(days) : days}
                    </span>
                    <span className={`text-sm ${isUrgent ? 'text-amber-600' : 'text-neutral-400'}`}>
                      days
                    </span>
                  </div>
                </div>
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-6 py-5 border-b border-neutral-100">
          ...
          {currentBlock && <p className="text-base text-neutral-500 mt-2">{currentBlock.title}</p>}
        </div>

        {isAdding && (
          <div className="p-6 border-b border-neutral-100 space-y-3">
            <input type="text" placeholder="Task name" value={newBlock.title} onChange={(e) => setNewBlock({ ...newBlock, title: e.target.value })}
              className="w-full px-3 py-2 text-sm bg-neutral-50 border border-neutral-200 rounded-lg focus:outline-none focus:border-neutral-300 placeholder:text-neutral-300" autoFocus />
            <div className="flex gap-2">
            ...
            <div className="flex gap-2">
              <button onClick={handleAdd} disabled={!newBlock.title} className="flex-1 py-2 text-sm text-neutral-600 hover:text-neutral-900 transition-colors disabled:text-neutral-300">Add</button>
        ...
        {sortedBlocks.length === 0 && !isAdding ? (
          ...
          <div className="divide-y divide-neutral-100">
            {sortedBlocks.map((block) => {
              ...
              return (
                <div key={block.id} className={`px-6 py-4 flex items-start gap-5 ${isPast ? 'opacity-40' : ''} ${isCurrent ? 'bg-neutral-50' : ''}`}>
                  <span className={`text-base tabular-nums ${isCurrent ? 'text-neutral-900 font-medium' : 'text-neutral-400'}`}>
                    {block.startTime}
                  </span>
                  <p className={`flex-1 text-base ${isCurrent ? 'text-neutral-900 font-medium' : 'text-neutral-600'}`}>{block.title}</p>
                  {isCurrent && <span className="text-xs text-rose-500 font-medium uppercase">Now</span>}
                </div>
              );
            })}
          </div>
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="text-center mb-6">
          <span className="font-serif text-6xl font-medium text-neutral-900 tabular-nums">{pct}</span>
          <span className="text-2xl text-neutral-300">%</span>
          <p className="text-sm text-neutral-400 mt-2 uppercase tracking-wider">Complete</p>
        </div>
          ...
          <div className="mt-5 pt-5 border-t border-neutral-100 space-y-2">
            {boards.slice(0, 3).map((board) => (
              <button key={board.id} onClick={() => onNavigate?.(board)}
                className="w-full flex items-center justify-between text-sm text-neutral-500 hover:text-neutral-900 transition-colors group">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-6 border-b border-neutral-100">
          <input
            ...
            className="w-full text-base bg-transparent focus:outline-none placeholder:text-neutral-300"
          />
        ...
        {captures.length === 0 ? (
          ...
          <div className="divide-y divide-neutral-100">
            {captures.slice(0, 5).map((c) => (
              <div key={c.id} className="px-6 py-4 hover:bg-neutral-50 transition-colors">
                <p className="text-base text-neutral-600">{c.text}</p>
                <p className="text-sm text-neutral-400 mt-1">{timeAgo(c.createdAt)}</p>
              </div>
            ))}
          </div>
        )}
            ...
            {recent.map((note) => (
              <button key={note.id} onClick={() => onNavigate?.(note)}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-neutral-400 mt-1">{getSectionName(note.parentId)} · {timeAgo(note)}</p>
                </div>
