        ...
        <nav className="flex items-center text-sm min-w-0">
          {breadcrumbs.map((crumb, index) => (
            <span key={index} className="flex items-center min-w-0">
              {index > 0 && (
                <ChevronRight size={14} className="mx-1 sm:mx-1.5 text-neutral-300 dark:text-neutral-600 flex-shrink-0" />
              )}
              {crumb.onClick ? (
                <button
                ...
                <span className="text-neutral-900 dark:text-neutral-100 truncate">{crumb.label}</span>
              )}
            </span>
          ))}
        </nav>
